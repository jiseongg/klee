...
generate_sandbox() {
	...
	if [[ ! -d "${script_base}/sandbox" ]]; then
		tar xvf "${script_base}/sandbox.tar" -C "${script_base}"
	fi
}
...
run_klee() {
	...
	local solver="--solver-backend=${solver} --use-forked-solver --use-cex-cache --use-query-log=solver:kquery"
	local debug_inst="--debug-print-instructions=all:file"
	...
	local env="--env-file=${script_base}/test.env --run-in-dir=${script_base}/sandbox"
	...
	ulimit -s unlimited && ulimit -n 1000000 && ulimit -c unlimited && \
		klee \
		${optimize} ${solver} ${libs} ${output} ${debug_inst} \
		${budgets} ${env} ${opt} ${search} \
